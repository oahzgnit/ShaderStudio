...
void main(){
    ...
    color += mix(vec3(0.075,0.114,0.329),vec3(0.973,0.843,0.675),circle(grid1,0.1)-circle(grid1,0.0001));

    vec2 grid2 = tile(st,4.);
    grid2 = tile(st + vec2(cos(u_time),sin(u_time))*0.02 ,5.);
    color = mix(color, vec3(0.761,0.247,0.102), circlePattern(grid2,0.5)) - circlePattern(grid2,0.05),
