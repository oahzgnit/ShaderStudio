...
void main() {
    ...
    float grid2 = box(offsetSt,vec2(0.5),0.01) + 2.*box(st,vec2(0.7),0.01);
    float grid3 = box(offsetSt,vec2(0.2),0.01) + 0.5*box(st,vec2(0.2),0.01);
    float grid = 1.0-X(movingTiles(st,15.+snoise(vec3(st,1.)), 0.1),0.05);

    gl_FragColor = vec4(vec3(grid*hsb2rgb(vec3(0.761))+grid2*vec3(0.123, 0.234, 0.456)+grid3*vec3(0.123, 0.234, 0.456)),1.0);
}