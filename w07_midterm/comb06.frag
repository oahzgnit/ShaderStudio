...
float circlePattern(vec2 st, float radius) {
    return  circle(st+vec2(0.28,-.5), radius)+
            circle(st+vec2(-0.28,.5), radius)+
            circle(st+vec2(-.1,0.5), radius)+
            circle(st+vec2(.1,-0.5), radius);
}

float circle2(vec2 st, float radius) {
    return  circle(st+vec2(0.0,-.1), radius)+
            circle(st+vec2(0.0,.1), radius)+
